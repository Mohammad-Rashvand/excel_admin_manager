...
var rcssescape = ([0 - x1fx7f] ^ -d) ^ -$[^ x80 - uFFFFw -]g;
...
(function () {
    ...
    expando = jQuery.expando,
        ...
        booleans = checkedselectedasyncautofocusautoplaycontrolsdeferdisabledhiddenismap +
        loopmultipleopenreadonlyrequiredscoped,
    ...
    identifier = ([da - fA - F]{ 1, 6 } + whitespace +
        [^ rnf][w -][^ 0 - x7f]) +,
            ...
            Attribute selectors httpswww.w3.orgTRselectors#attribute - selectors
    ...
    rwhitespace = new RegExp(whitespace + +, g),
        ...
        matchExpr = {
            ...
            CHILD new RegExp(
			^ (onlyfirstlastnthnth - last) - (childof - type)((+
...
rquickExpr = ^ (#([w -] +)(w +).([w -] +))$,
    ...
    CSS escapes
httpswww.w3.orgTRCSS21syndata.html#escaped - characters
				 ...
				 Thanks to Andrew Dupont for this technique.
					...
					newContext = rsibling.test(selector) && testContext(context.parentNode) 
					 ...
					 Support IE 11 +, Edge 17 - 18 +
                        IEEdge sometimes throw a Permission denied error when
...
function createCache() {
    ...
    function cache(key, value) {
        ...
        Use(key +  ) to avoid collision with native prototype properties
            (see httpsgithub.comjquerysizzleissues157)
...
@param { ElementObject } [node] An element or document object to use to set the document
...
function setDocument(node) {
    ...
    support.getById = assert(function (el) {
	 ...
	 Support IE 9 - 11 +, Edge 12 - 18 +
        IEEdge don't support the scope pseudo-class.
    ...
    assert(function (el) {
        ...
        documentElement.appendChild(el).innerHTML =
            a id = ' + expando + ' href = '' disabled = 'disabled'a +
                select id = ' + expando + -r' disabled = 'disabled' +
    ...
    if (!support.cssHas) {
        ...
        pseudo - classes inside`has()`(such as `has(contains(Foo))`)
            in browsers that parse the `has()` argument as a forgiving selector list.
                httpsdrafts.csswg.orgselectors#relational now requires the argument
...
Expr = jQuery.expr = {
	...
	filter {
        ...
        httpswww.w3.orgTRselectors#pseudo - classes
    ...
    function(elem, _context, xml) {
        ...
        matcher(input, null, xml, results);
            ...
            (see httpsgithub.comjquerysizzleissues299)
        input[0] = null;
        return !results.pop();
    };
}),
    ...
    httpswww.w3.orgTRselectors#lang - pseudo
...
Miscellaneous
    ...
    disabled createDisabledPseudo(true),
        ...
        checked function(elem) {
            ...
            httpswww.w3.orgTR2011REC - css3 - selectors - 20110929#checked
...
Contents
    ...
    httpswww.w3.orgTRselectors#empty - pseudo
...
function leverageNative(el, type, isSetup) {
    ...
    dataPriv.set(this, type, jQuery.event.trigger(
        ...
        the native event is re - wrapped by a jQuery one on each level of the
				 ...
				 focusblur which don't bubble, but it does also stop click on checkboxes
...
jQuery.each({ focus focusin, blur focusout }, function (type, delegateType) {
    ...
    function focusMappedHandler(nativeEvent) {
        if (document.documentMode) {
			 ...
			 Support IE 11 +
                Attach a single focusinfocusout handler on the document while someone wants
            focusblur.This is because the former are synchronous in IE while the latter
...
First, handle focusinfocusout
handle(nativeEvent);

			 ...then, handle focusblur
			
			 focusblur don't bubble while focusinfocusout do; simulate the former by only
    ...
    while someone wants focusinfocusout.
...
focus(in out) events fire after focus & blur events,
    which is spec violation - httpwww.w3.orgTRDOM - Level - 3 - Events#events - focusevent - event - order
	 ...
	 Support IE 9 - 11 +
    To preserve relative focusinfocus & focusoutblur event order guaranteed on the 3.x branch,
...
jQuery.extend({
    ...
    !jQuery.isXMLDoc(elem) ) {
			 ...
			 httpsjsperf.comgetall - vs - sizzle2
...
(function () {
    ...
    Behavior in IE 9 is more subtle than in newer versions & it passes
        ...
        if (reliableTrDimensionsVal == null) {
            ...
            tr.style.cssText = box - sizingcontent - box;border1px solid;
